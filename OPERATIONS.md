...
2. 도구 키워드(`TOOL_KEYWORDS`) — 제목에 1개 이상, 또는 초록에 **서로 다른 용어
   2개 이상**. 어떻게 푸는가.
...
**도구 판정은 반드시 `TOOL_RE`(단어 경계)로 한다.** 부분 문자열로 찾으면
`gis`가 `registration`·`logistic`에, `shap`이 `shape`·`shaping`에 걸린다.
실제로 이 버그 때문에 공간분석을 전혀 쓰지 않은 사회조사 논문이 여러 편
게시됐다. 초록에 한 번 스쳐 나오는 정도로 통과시키지 않는 것도 같은 이유다.

주제·도구 탈락은 로그에 `주제 무관 N건, 도구 미사용 N건`으로 남는다.
이 로그가 없던 동안 위 버그를 오래 눈치채지 못했다.
...
**후보가 크게 준다.** 대상지 기준과 도구 게이트를 더하면서 주제당 후보가
48건에서 6~13건까지 떨어졌다. `OPENALEX_PAGES`를 3(최대 600건)으로 올렸지만
**효과는 +2건에 그쳤다.** 로그를 보면 탈락 690건 중 490건이 "주제 무관"이라,
뒤 페이지로 갈수록 주제와 먼 논문이기 때문이다.

`title_and_abstract.search`로 질의를 좁히는 것도 시험했다. 적중률은 6% → 20%로
오르지만 모집단이 13,208건에서 61건으로 줄어 상한이 오히려 낮아진다. 쓰지 않는다.
...
3. 그래도 안 되면 차순위 논문으로 넘어간다(`MAX_FULLTEXT_ATTEMPTS`, 한 주제당
   최대 15건). 점수차 마진은 두지 않는다 — 전문이 필수라 마진 때문에 포기하면
   게시 자체가 불가능해지고, 저널 품질 기준이 이미 하한을 보장한다.
   한 주제에서 전부 막히면 다음 주제로 넘어가고, 네 주제 모두 실패하면
   그날은 게시를 건너뛴다. **초록 기반 게시는 하지 않는다.**
